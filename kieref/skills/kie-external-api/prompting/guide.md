...
3. If the workflow needs reference images, get **hosted URLs** (KIE uses `imageUrls` / `image_input` — no presigned upload flow). For local files, upload just the ones this run needs via `./scripts/imgbb-upload.sh <file>`. See `SKILL.md` → *Reference images: hosted URLs, not file uploads*.
