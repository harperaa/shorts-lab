...
1. Upload **just the files this run needs**, when it needs them:
   `./scripts/imgbb-upload.sh <file> [file ...]` → prints one public URL per line.
   Results are cached by sha256 in `logs/imgbb-cache.jsonl`, so repeat calls don't re-upload.
2. Pass the hosted URL into `imageUrls` (Veo) or `input.image_input` (jobs).
3. Never bulk-upload the `references/` tree — upload inline, per run.
